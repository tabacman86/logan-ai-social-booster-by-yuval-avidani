...
async function generateComment(postContent, commentStyle = 'professional') {
    try {
        ...
        // Detect if post is in Hebrew
        const isHebrew = /[\u0590-\u05FF]/.test(postContent);
        const languageInstruction = isHebrew ? 'Respond in Hebrew' : 'Respond in English';
        
        const prompt = `${stylePrompts[commentStyle]} in response to this social media post. ${languageInstruction}. Keep it concise (1-2 sentences), authentic, and engaging. Avoid emojis and hashtags. Post content: "${postContent.substring(0, 500)}"`;

        console.log('Sending request to Cohere with prompt:', prompt.substring(0, 200) + '...');
        ...
        if (data.generations && data.generations.length > 0) {
            ...
            comment = comment.replace(/\s+/g, ' '); // Replace multiple spaces with single space
            comment = comment.trim();
            
            // Remove common prefixes that AI might add
            comment = comment.replace(/^(Comment:|Response:|Reply:)\s*/i, '');
            
            console.log('Generated clean comment:', comment);
            return comment;
        } else {
            throw new Error('No comment generated by Cohere');
        }
    } catch (error) {
